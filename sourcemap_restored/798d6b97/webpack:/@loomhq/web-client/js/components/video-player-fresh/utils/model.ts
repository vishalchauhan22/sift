...
export const useParseReaction = (): ((
  ...
  const parseFn = useCallback(
    (reaction: GraphQlReactionFromServer) => {
      const viewingUserId = isLoggedIn ? userId : getAnonUserId();
    ...
    [userId, anonUserName, video.processingInformation?.trimRanges, isLoggedIn]
  );
...
export function parseVideo(video: VideoFromServer): VideoModel {
  ...
  return {
    ...
    calendarMeetingGuid: video.calendarMeetingGuid,
    ...
    processingInformation: {
      ...
      trimProgress: video.processing_information.trim_progress,
    ...
    videoProperties: {
      ...
      externalUpload: properties.externalUpload,
