...
export const useVideoTextReplacements = ({
  ...
  useConsolidatedEditVideoTextReplacementsUpdatedSubscription({
    variables: { videoId },
    onData: ({ data }) => {
      // If no callback is provided, we don't need to do anything
      if (!onPlayableContentChange) {
        return;
      }

      if (
        data?.data?.videoTextReplacementsUpdated?.__typename !==
        'RegularUserVideo'
      ) {
        return;
      }

      const newVideoTextReplacements =
        data.data.videoTextReplacementsUpdated.textReplacements;

      if (
        hasPlayableContentChanged({
