...
import {
  ...
  isExpectedUnsuccessfulTranscriptionStatus,
...
const memoizedFetchFromSourceUrl = memoizeOne(fetchFromSourceUrl);
...
export const useTranscript = (): UseTranscriptReturnType => {
  ...
  const {
    setTranscript: setTranscriptInStore,
    retranscriptionStatus,
    setRetranscriptionStatus,
    resetTranscription,
  } = useTranscriptStore();
  ...
  const {
    ...
    onCompleted: completedData => {
      if (completedData?.fetchVideoTranscript?.__typename === 'GenericError') {
        showErrorBar({
      ...
      if (
        ...
        // don't do anything with invalid requests
        // for example, if there is no video found for the given videoId
        // or if the viewer does not have permission to access the transcript
        // an error bar is unnecessary
        return;
      }
    ...
    unsubscribeRef.current =
      subscribeToMore<TranscriptStatusChangedSubscription>({
        ...
        updateQuery: (prev, { subscriptionData }) => {
          ...
          const transcriptionFinished =
            isTerminalTranscriptionStatus(newTranscriptStatus);
  ...
  // From the UI's perspective, a transcript that is unsuccessful/unavailable is still a success, shortcut
  // the success marker for both transcript and captions so they don't time out
  const markSuccess = useMarkRUMSuccess();
  const hasMarkedSuccess = useRef(false);
  useEffect(() => {
    if (
      status &&
      isExpectedUnsuccessfulTranscriptionStatus(status) &&
      !hasMarkedSuccess.current
    ) {
      markSuccess(SuccessMarkers.TranscriptLoaded);
  ...
  useEffect(() => {
    const fetchTranscript = async () => {
      ...
      const transcriptResponse = await memoizedFetchFromSourceUrl(
        sourceUrl,
        videoId
      );
      ...
      // function processes the transcript data and create phrases and search indexes
      setTranscriptInStore(transcriptResponse);

      // Given the transcript tab is not necessarily visible, this is our best indication
      // of transcript being successfully loaded and available
