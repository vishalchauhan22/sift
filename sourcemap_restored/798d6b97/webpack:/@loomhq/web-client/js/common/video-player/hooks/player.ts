...
import { useSkipPrePlayState } from './useSkipPrePlayState';
  ...
  React.useEffect(() => {
    if (
      ...
      openModal({
        ...
        options: {
          videoOwnerName,
          source: ASG_SOURCES.SignedOutHardGatingViews,
          signupParams: {
            signup_source: ASG_SOURCES.SignedOutHardGatingViews,
          },
...
export function usePlayerHasStarted(videoId: string): boolean {
  ...
  const { skipPrePlayState } = useSkipPrePlayState();
  ...
  return hasStarted || skipPrePlayState;
}
