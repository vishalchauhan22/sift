...
export const DefaultPlayBar = React.memo(({ videoId }: { videoId: string }) => {
  ...
  const showCommentForm = isOpen && !hideCommentInputForAnonMarketingLoom;
  const showPlayBarContent = !isOpen || hideCommentInputForAnonMarketingLoom;
...
export const EmbedPlayBar = React.memo(({ videoId }: { videoId: string }) => {
  ...
  return (
    <PlayBarWrapper ref={ref} commentformIsOpen={isOpen}>
      {isOpen ? (
        <CommentFormInput videoId={videoId} />
      ) : (
