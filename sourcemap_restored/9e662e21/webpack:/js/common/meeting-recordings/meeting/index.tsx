...
import { useUpdateCalendarMeetingRecordMutation } from './UpdateCalendarMeetingRecord.generated';
...
interface MeetingProps {
  ...
  emailDomains: string[];
  readOnly?: boolean;
  openModalForMeetingId: string | number | null;
  setOpenModalForMeetingId: (calendarMeetingId: string | number | null) => void;
...
export const Meeting = ({
  ...
  emailDomains,
  readOnly = false,
  openModalForMeetingId,
  setOpenModalForMeetingId,
  source,
  ...
  const [updateCalendarMeetingRecordMutation] =
    useUpdateCalendarMeetingRecordMutation();
  ...
  const toggleRecord = (event: SyntheticEvent) => {
    ...
    updateCalendarMeetingRecordMutation({
      variables: {
      ...
      onCompleted(data) {
        if (
          data.updateCalendarMeetingRecord &&
          (data.updateCalendarMeetingRecord.__typename !==
            'UpdateCalendarMeetingRecordPayload' ||
            !data?.updateCalendarMeetingRecord?.success)
        ) {
          // TODO: Replace with localised error text
          // Toasts should be for successes only
          setShowConfirmationToast(ERROR_MESSAGE);
  ...
  const showMeetingSettings = !pastMeeting && currentUserRecording;
  ...
  return (
    ...
    >
              ...
              <div>
                <Split>
                  {meeting.past ? (
                    ...
                    <Text variant="title" hasEllipsis ellipsisLines={2}>
                      <a
                        href={`${meeting.url}`}
                ...
                <Arrange gap="12px" justifyContent="stretch">
                  {showMeetingSettings && (
                    <IconButton
                      ...
                      icon={<SvgSettings />}
                      onClick={() => setOpenModalForMeetingId(meeting.id)}
                    />
                  )}
                  <MeetingNotesButton
                    meeting={meeting}
      ...
      <MeetingShareModal
        ...
        onClose={() => setOpenModalForMeetingId(null)}
        open={openModalForMeetingId === meeting.id}
