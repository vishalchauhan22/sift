...
import {
  captionsStore,
  isLanguageValid,
} from '../video-player/hooks/captionsStore';
...
export const useTranscript = (): UseTranscriptReturnType => {
  ...
  const {
    ...
    onCompleted: completedData => {
      if (
        ...
        const errorMessage = completedData?.fetchVideoTranscript?.message;
  ...
  const isValidStoredLanguage =
    storedLanguage && isLanguageValid(storedLanguage);
  const hasValidLanguageSelection =
    captionsLanguageSelection || isValidStoredLanguage;
