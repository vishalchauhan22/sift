...
export const ViewerAvatar = (props: {
  ...
  const imageSrc = props.avatar ? getAvatarFromPath(props.avatar) : undefined;
  const nameProp = props.name;
  const avatarName = nameProp !== ANONYMOUS_NAME ? nameProp : undefined;

  return (
    <UserAvatar
      avatarSize={props.size ?? 4}
      avatarSrc={imageSrc ?? undefined}
...
export const ViewerName = (props: { name: string | null }): JSX.Element => (
  <div>
...
export const ViewsList = (props: { views: number[] }): JSX.Element => {
  return (
    <>
...
function getEmojiIcon({
  getEmojiUnicodeByName,
  action,
  evt,
}: {
  getEmojiUnicodeByName: (emoji: string) => string;
  action: string;
  evt: any;
}): JSX.Element {
...
function sortCompareByTimestamp(
  { ts }: { ts: number },
  { ts: ts2 }: { ts: number }
) {
  if (ts > ts2) {
