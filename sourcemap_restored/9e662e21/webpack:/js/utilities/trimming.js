...
export const waitingForTrim = video => {
...
export const createCurrentTrimRange = (currentTime, duration, trimRanges) => {
  let currentTrimRangeStart = currentTime;
  let currentTrimRangeEnd;

  let trimRangeAfterStart = null;
  let trimRangeBeforeStart = null;
  ...
  currentTrimRangeEnd = Math.min(
    ...
    duration,
    trimRangeAfterStart ? trimRangeAfterStart.from - 1 : duration
  );
  ...
  if (
    ...
    currentTrimRangeStart = Math.max(
      currentTrimRangeStart - duration * DEFAULT_TRIM_RANGE_LENGTH,
      trimRangeBeforeStart ? trimRangeBeforeStart.to + 1 : 0
    );
  }
