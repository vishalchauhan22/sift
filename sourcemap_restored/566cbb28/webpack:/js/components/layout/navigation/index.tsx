...
const getNavbarZIndex = (
  ...
  // When the drawer is explicitly opened  ensure the nav bar overlays header/content
  if (isDrawerOpen) {
    return 103;
  }
...
function NavigationWithoutFeatureWrapper({
  ...
  useEffect(
    () => {
      if (isShareVideo && isMobile) {
        // defensive try-catch to prevent share page from crashing
        try {
          toggleSidebar(true, { updatePreference: false });
        } catch (error) {
          console.error(
            'Failed to collapse navigation on share video page:',
            error
          );
        }
        return;
