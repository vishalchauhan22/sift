...
import { VideoFromGraphQl, parseVideoEmbed, userContext } from './utils/model';
import {
...
import { TitleBarDark } from '@js/common/video-player/components/title-bar-dark';
...
export const SharePageVideo = ({
  ...
  const { isExpVizCohesionShareTitle, expVizCohesionShareTitleVariant } =
    useExpVizCohesionShareTitle(userCanEdit);
  const isDesktop = useMedia(['(min-width: 768px)'], [true], false);
  const showTitleUnderVideo = isExpVizCohesionShareTitle && isDesktop;
  const showViewsUnderVideo = expVizCohesionShareTitleVariant === 'variant-2';
  const thumbnail = video.thumbnails.staticFullUrl;

  return (
    <article
      {...(showTitleUnderVideo ? { className: styles.videoWrapper } : null)}
    >
      ...
      {showTitleUnderVideo ? (
        <TitleBarDark
          ...
          thumbnail={thumbnail}
          videoId={video.id}
          showViewsUnderVideo={showViewsUnderVideo}
        />
      ) : null}
    </article>
  );
