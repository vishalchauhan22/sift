...
import { shouldShowSpeakerName } from '@js/common/transcripts/should-show-speaker-name';
...
// in phrases transcript array (searches both phrase value and speaker name when visible)
export const findIndicesOfSearchPhraseInPhrases = (
  ...
  try {
    ...
    phrases.forEach((phraseObj, index) => {
      ...
      const speakerName = phraseObj.speakerName || '';
      let counter = 0;

      // Always search in phrase value
      ...
      // Only search in speaker name if it would be visible to the user
      const prevPhrase = index > 0 ? phrases[index - 1] : undefined;
      const speakerNameVisible = shouldShowSpeakerName(
        speakerName,
        index,
        prevPhrase
      );

      if (speakerNameVisible) {
        while (regex.exec(speakerName)) {
          result.push({ index, counter });
          counter++;
        }
      }
