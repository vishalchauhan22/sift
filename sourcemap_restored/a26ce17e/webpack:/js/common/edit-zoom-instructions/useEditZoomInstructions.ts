...
export const useEditZoomInstructions = (
  ...
  supportsEditZoomInstructions: boolean;
  ...
  const { zooms, supportsEditZoomInstructions } = useMemo(() => {
    if (data?.getVideo?.__typename !== 'RegularUserVideo') {
      return { zooms: [], supportsEditZoomInstructions: false };
    }

    const supportsEditZoomInstructions =
      data.getVideo.supportsEditZoomInstructions;
    const zooms = supportsEditZoomInstructions
      ? data.getVideo.editZoomInstructions
      : [];
