...
export function IdentityMigrationBanner({
  ...
  if (
    currentUser !== null &&
    currentUser.identityMigrationEligibleDate !== null
  ) {
    const bannerNotificationStartDate = new Date(
      new Date(currentUser.identityMigrationEligibleDate).getTime() -
        7 * 24 * 60 * 60 * 1000
    );

    withinBannerWindow = bannerNotificationStartDate.getTime() <= now.getTime();
  }
  const showIdentityMigrationBanner = withinBannerWindow && !atlassianMastered;
