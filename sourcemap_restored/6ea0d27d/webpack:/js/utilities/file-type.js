...
// we import file-type explicitly on the frontend because Sindre doesn't
// pre-transpile his modules for use with es5 environments (and we support IE11):
// https://github.com/sindresorhus/file-type/issues/70#issuecomment-268853935
...
const fileType = input => {
  if (!(input instanceof Uint8Array || Buffer.isBuffer(input))) {
    throw new TypeError(
      `Expected the \`input\` argument to be of type \`Uint8Array\` or \`Buffer\`, got \`${typeof input}\``
    );
  }
    ...
    if (
      check(
        [
          ...
          0x6c, 0x69, 0x63, 0x61, 0x74, 0x69, 0x6f, 0x6e, 0x2f, 0x65, 0x70,
          0x75, 0x62, 0x2b, 0x7a, 0x69, 0x70,
        ],
        { offset: 30 }
      )
    ) {
