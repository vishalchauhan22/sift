...
export const VideoPlayer = ({ videoId, ...props }: VideoProps): JSX.Element => {
  ...
  // Start:EXP_VISUAL_REFRESH_ON_ANON_SHARE_PAGE_GATE_V2
  const { status } = usePlayingStatus(videoId);

  const isPaused = status === 'paused';
  const { isExpAnonSharePageGateRefresh } = useExpAnonSharePageGateRefresh();
  // End: EXP_VISUAL_REFRESH_ON_ANON_SHARE_PAGE_GATE_V2
      ...
      <div
        className={classNames({
          [styles.onSignedOutPauseOverlayLight]:
            isPaused && isExpAnonSharePageGateRefresh,
        })}
      />
      {/* End: EXP_VISUAL_REFRESH_ON_ANON_SHARE_PAGE_GATE_V2 */}
