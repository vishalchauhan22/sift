...
import { FailType, MarkerName, RUMMark } from './types';
...
export const RUMProvider = <T extends RUMReportingContext>({
  ...
  useEffect(() => {
    if (allSuccessMarks.length) {
      const mostRecentMark = allSuccessMarks[allSuccessMarks.length - 1];

      if (
        reportableSuccessMarks[mostRecentMark.name] &&
        reportableSuccessMarks[mostRecentMark.name] !== mostRecentMark
      ) {
