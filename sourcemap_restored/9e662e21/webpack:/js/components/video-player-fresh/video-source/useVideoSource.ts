...
export function useVideoSourceFromGraphql({
  ...
  isPlaylistValidationEnabled?: boolean;
}): VideoSourceState {
  ...
  const [{ source, initialMimeType }, setSourceState] =
  ...
  const [runQuery] = useGetVideoSourceLazyQuery({
    ...
    onCompleted: async data => {
      ...
      try {
        if (!data.getVideo || data.getVideo.__typename !== 'RegularUserVideo') {
          logger.error(
            new Error('Error fetching video source - invalid response'),
            {
              videoId,
              recordingVersion,
              getVideoData: JSON.stringify(data.getVideo),
            },
            { feature: Feature.VideoPlayer }
          );
        ...
        const videoSource = await processVideoSource(
          url,
          {
        ...
        setSourceState(sourceStateUpdater(videoSource));
      } catch (error) {
        logger.error(
          error,
          { videoId, recordingVersion },
          { feature: Feature.VideoPlayer, source: 'useVideoSource' }
        );
