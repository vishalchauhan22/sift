...
const PlayBarContent = ({ videoId }: { videoId: string }) => {
  ...
  const collapsedSettingsBreakpoint = airPlayIsEnabled ? 650 : 600;
  const showCollapsedSettings = width < collapsedSettingsBreakpoint;
...
const PlayBarContentEmbed = ({ videoId }: { videoId: string }) => {
  ...
  const collapsedSettingsBreakpoint = () => {
    if (airPlayIsEnabled) {
      return 700;
    }
