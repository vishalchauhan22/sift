...
import { useVideoTrimProgressUpdatedSubscription } from '@js/utilities/VideoTrimProgressUpdated.generated';
import { useRegenerateMp4Mutation } from '@js/common/video/RegenerateMP4.generated';
import { RegenerationType } from '@js/globalTypes.generated';
import { useFlagIsActivated } from '@js/hooks/featureFlag';
import {
  FEATURE_GATES,
  ControlType,
} from '../../../../../libraries/shared-utilities/src/constants/statsig';
...
export function useHandleSource(
  ...
  const [mp4RegenerationStartTime, setMP4RegenerationStartTime] = useState<{
    time: number | null;
  }>({ time: null });

  const [hasUnrecoverableError, setHasUnrecoverableError] =
  ...
  const {
    ...
    setForceMp4,
    setSource,
    shouldRegenerateMp4: failedToGetMp4SourceUrl,
  ...
  const onUnrecoverableError = useCallback(() => {
    ...
    setForceMp4(true);
  }, [setForceMp4, player, setHasUnrecoverableError]);

  const mp4RegenerationEnabled = useFlagIsActivated({
    flag: FEATURE_GATES.LOOM_MP4_REGENERATION_FROM_PLAYER_ENABLED,
    controlType: ControlType.STATSIG_FEATURE_GATE,
    defaultValue: false,
    activationValues: [true],
  });

  const [regenerateMp4Mutation, { called: regenerateMP4MutationCalled }] =
    useRegenerateMp4Mutation({
      variables: {
        input: {
          videoId: video.id,
          regenerationType: RegenerationType.Mp4Fallback,
        },
      },
    });

  const { data: trimProgressData } = useVideoTrimProgressUpdatedSubscription({
    variables: {
      videoId: video.id,
    },
  });
  ...
  const reportPlayerEvent: ReportPlayerEventFn = (
    ...
    if (
      ...
      (eventName === PLAYER_EVENTS.LOADING_STARTED ||
        (eventName === PLAYER_EVENTS.PLAYER_ERROR &&
          extras.fatal &&
      ...
      sendVideoPlaybackEvent({
        ...
        videoWorkspaceLoomOrgId: video.organizationId,
        playerContext,
      });
    }
  };
  ...
  const reportMp4RegenerationTime = () => {
    if (mp4RegenerationStartTime.time) {
      const delta = Date.now() - mp4RegenerationStartTime.time;
      reportPlayerDistribution(PLAYER_EVENTS.MP4_REGENERATION, delta);
      setMP4RegenerationStartTime({ time: null });
    }
  };

  const reportMp4RegenerationTimeRef = React.useRef(reportMp4RegenerationTime);

  React.useEffect(() => {
    reportMp4RegenerationTimeRef.current = reportMp4RegenerationTime;
  }, [reportMp4RegenerationTime]);

  React.useEffect(() => {
    //we only want to reset the playback URL when there is no MP4 source URL
    if (!failedToGetMp4SourceUrl) {
      return;
    }
    if (trimProgressData?.videoTrimProgressUpdated?.transcodedUrl) {
      processVideoSource(
        trimProgressData.videoTrimProgressUpdated.transcodedUrl,
        {}
      )
        .then(videoSource => {
          setSource({
        ...
        .finally(() => {
          reportMp4RegenerationTimeRef.current();
        });
    }
  }, [trimProgressData, initialMimeType, setSource, failedToGetMp4SourceUrl]);

  //we do not want to allow re-generation of mp4 if the trim progress is -1,
  //meaning the trim has failed for this trimId
  const videoHasNoFailuresOnTrim =
    video.processingInformation?.trimProgress !== -1;

  const timeAfterLatestTrim = video.processingInformation?.trimId
    ? Date.now() - video.processingInformation.trimId
    : 15001;
  //allow 15 seconds for trim progress to start reporting to the frontend after video creation or trim
  const isRightAfterTrimOrCreation =
    timeAfterLatestTrim < 15000 || !video.complete;

  React.useEffect(() => {
    if (
      failedToGetMp4SourceUrl &&
      !regenerateMP4MutationCalled &&
      videoHasNoFailuresOnTrim &&
      mp4RegenerationEnabled &&
      !isRightAfterTrimOrCreation &&
      !trimProgressData
    ) {
      regenerateMp4Mutation();
      setMP4RegenerationStartTime({ time: Date.now() });
    }
  }, [
    failedToGetMp4SourceUrl,
    regenerateMP4MutationCalled,
