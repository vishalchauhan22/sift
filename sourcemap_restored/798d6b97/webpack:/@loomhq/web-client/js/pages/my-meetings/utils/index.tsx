...
/**
 * Determines if meeting settings should be shown for a given meeting and current user.
 * Settings modal is only available for upcoming meetings that the current user is recording.
 */
export function shouldShowMeetingSettings(
  meeting: CalendarMeeting | null | undefined,
  ...
  const pastMeeting = Boolean(meeting.past || meeting.videoId);
  const currentUserRecording =
    meeting.recorder && String(meeting.recorder.id) === String(currentUserId);

  return !pastMeeting && Boolean(currentUserRecording);
}

/**
 * Checks if a meeting can have its settings opened from URL parameters.
 * Returns true if the meeting exists and user has access, false otherwise.
 */
export function canOpenMeetingSettingsFromUrlParam(
  meetingGuidFromParam: string,
  meetings: CalendarMeeting[] | undefined,
  currentUserId: string | number | undefined
): boolean {
  if (!meetings || meetings.length === 0 || !currentUserId) {
    return false;
  }

  const targetMeeting = meetings.find(
    m => String(m.calendarMeetingGuid) === String(meetingGuidFromParam)
  );
  ...
  return shouldShowMeetingSettings(targetMeeting, currentUserId);
}
