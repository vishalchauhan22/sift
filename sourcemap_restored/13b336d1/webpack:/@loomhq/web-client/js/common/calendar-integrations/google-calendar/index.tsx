...
import { Arrange, Button, Icon, Text, Toast } from '@loomhq/lens';
...
import { SvgWhiteGoogle } from './google';
...
interface ConnectGoogleCalendarProps {
  ...
  handleClick?: () => void;
  shouldRedirect?: boolean; //redirects to /meetings/settings
  shouldRedirectToSetup?: boolean; //redirects to /meetings/setup
  googleVariant?: boolean;
  size?: ButtonSize;
...
export const ConnectGoogleCalendar = ({
  ...
  size = 'medium',
  hasFullWidth = false,
}: ConnectGoogleCalendarProps): JSX.Element | null => {
  ...
  const { googleCalendarConnected } = useGetCalendarsConnected();

  const connectGoogleCalendar = () => {
    openOAuthPopup(`google-calendar`, {}, data => {
      if (data.error) {
        showErrorBar({
          message: data.error,
        ...
        if (shouldRedirect) {
          window.location.href = '/meetings?showSettings=1';
        }
  ...
  const buttonVariant = getButtonVariant(variant, googleCalendarConnected);
  const isBranded = isBrandedVariant(variant);
  const connectButtonLabel = isBranded ? 'Connect Google Calendar' : 'Connect';
    ...
    <>
      ...
      >
        <Arrange alignItems="start" gap="small">
          <Text>Google Calendar has been disconnected.</Text>
        </Arrange>
      </Toast>
      ...
      <Button
        ...
        icon={
          !googleCalendarConnected && isBranded ? (
            <Icon
              icon={
      ...
      >
        {googleCalendarConnected ? 'Disconnect' : connectButtonLabel}
      </Button>
    </>
  );
};
