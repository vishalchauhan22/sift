...
class AnalyticsEntityId {
  ...
  getAttrWithOriginalValue() {
    return {
      [this.attrName]: this.value
    };
  }
  getError() {
    if (this.format === "any" || !this.value || Array.isArray(this.value) && this.value.length === 0) {
      return null;
    }
    const valueType = typeof (Array.isArray(this.value) ? this.value[0] : this.value);
    if (valueType !== this.format) {
      return "".concat(this.attrName, " expects ").concat(this.format, ", but got ").concat(valueType);
    }
    return null;
  }
