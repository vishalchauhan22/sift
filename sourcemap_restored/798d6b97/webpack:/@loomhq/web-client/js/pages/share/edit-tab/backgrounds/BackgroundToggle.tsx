...
const BackgroundToggle = ({
  videoId,
  isUnavailable,
}: BackgroundToggleProps): JSX.Element => {
...
export const BackgroundToggleWithFeatureFlagCheck = ({
  videoId,
  isUnavailable,
}: BackgroundToggleProps): JSX.Element | null => {
  // For A/A test exposure
  useFeatureFlagValue(
    EXPERIMENTS.EXP_SHARE_PAGE_BACKGROUND_PICKER_AA,
    ControlType.STATSIG_EXPERIMENT
  );

  const sharePageBackgroundPickerFlagValue = useFeatureFlagValue(
    EXPERIMENTS.EXP_SHARE_PAGE_BACKGROUND_PICKER,
    ControlType.STATSIG_EXPERIMENT
  );
  const inSharePageBackgroundPickerExperiment =
    sharePageBackgroundPickerFlagValue === 'variant' ||
    sharePageBackgroundPickerFlagValue === 'variant-excluded';
  return inSharePageBackgroundPickerExperiment ? (
    <BackgroundToggle videoId={videoId} isUnavailable={isUnavailable} />
  ) : null;
};
