import { AVSERVER_SEGMENT_SPLITTING } from '@loomhq/shared-utilities/constants/featureFlag';
...
type SetupShakaProps = {
  ...
  isLoggedIn: boolean;
  setShouldRegenerateMp4: (boolean) => void;
};
...
export function setupShaka({
  ...
  //Shaka 1003 is a HTTP timeout from the server
  const shouldNotRegenerateMp4 = (error: ShakaErrorDetail): boolean => {
    return is5xxFromAvServer(error) || error.code === 1003;
  };
  ...
  ...
   */
  const is5xxFromAvServer = (error: ShakaErrorDetail) => {
    return (
      error.code === 1001 &&
      error.data &&
      error.data.length >= 2 &&
      typeof error.data[1] === 'number' &&
      error.data[1] >= 500 &&
      error.data[1] < 600
    );
  };

  const triggerUnrecoverableError = (error?: ShakaErrorDetail): void => {
    if (error && shouldNotRegenerateMp4(error)) {
      setShouldRegenerateMp4(false);
    }
    onUnrecoverableError();
  };

  const onLoadError = async (error: ShakaErrorDetail) => {
    logShakaErrorDetailProperties(error);
    ...
    if (!isSupportedBrowser(uaInfo.UABrowserName)) {
      const { sourceUrl, ...contextWithoutSourceUrl } = context;
      loggerx.info(
        'Shaka error for unsupported browser',
        {
          ...contextWithoutSourceUrl,
          videoId,
        },
        {
    ...
    triggerUnrecoverableError(error);
  };
  ...
  const createShaka = () => {
    ...
    try {
      ...
      shaka.addEventListener('error', async (error: ShakaError) => {
        logShakaErrorDetailProperties(error.detail);
        ...
        destroyMseTech(player).then(() => {
          triggerUnrecoverableError(error.detail);
        });
      });
...
const logShakaErrorDetailProperties = (detail: ShakaErrorDetail): void => {
  /**
   * Temporary logging to identify case where properties are missing
   */
  if (
    !detail.code ||
    !detail.severity ||
    !detail.category ||
    detail.handled === undefined
  ) {
    loggerx.warning('Shaka error detail missing properties', {
      code: detail.code,
      severity: detail.severity,
      category: detail.category,
      handled: detail.handled,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      message: (detail as any).message, // checking if we are receiving vanilla JS error
    });
  }
};
