...
export function useSeekPreview(
  ...
  const [tiles, setTiles] = useState<ScrubberThumbnailTimeMapping[]>();
  ...
  useEffect(() => {
    if (tiles?.length && !hasMarkedSuccess.current) {
      markSuccess(SuccessMarkers.SeekPreviewLoaded);
      hasMarkedSuccess.current = true;
    }
  }, [markSuccess, tiles?.length]);
