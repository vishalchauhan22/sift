...
export const DropMenu = onClickOutside(
  ...
  class Menu extends React.Component {
    handleClickOutside(e) {
      if (
        !this.props.open ||
        MENU_TRIGGER_CLASSES.includes(e.target.className)
      ) {
        return;
      }
    ...
    render() {
      const { className, open, items, stateProps } = this.props;
      const classNames = classnames('navigationBar__menu', className, { open });

      return (
        <ul className={classNames}>{handleItems(items, stateProps, null)}</ul>
      );
    }
  }
);
